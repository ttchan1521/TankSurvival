...
public class UnitAI : Unit
{
    ...
    {
        if (agent != null)
        {   //if the unit is using navmeshagent, set the target's position as the destination
            agent.speed = moveSpeed * GetSpeedMultiplier();
            agent.SetDestination(target.thisT.position);
        }
        else
        {   //otherwise just call the function in this script to move towards the target
    ...
    {   //normalized according to angle to be used in a lerp function
