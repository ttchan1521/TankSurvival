...
namespace TDSTK{
	...
	public class ShootObject : PooledObject {
		...
		public void Init() {
			...
			if (shootEffect)
			{
				if (!shootEffect.TryGetComponent<PooledObject>(out var pooledObject))
				{
					pooledObject = shootEffect.AddComponent<PooledObject>();
				}
			}

			if (hitEffect)
			{
				if (!hitEffect.TryGetComponent<PooledObject>(out var pooledObject))
				{
					pooledObject = hitEffect.AddComponent<PooledObject>();
				}
			}
		}
		...
		void OnTriggerEnter(Collider col){
			...
			if(col!=null && col.gameObject.layer==TDS.GetLayerCollectible()){
				...
				col.GetComponent<Collectible>().ReturnToPool();
				return;
			}
		...
		void HitEffect(Vector3 pos){
			...
			pooledObject.GetPoolItem(pos, Quaternion.identity).ReturnToPool(destroyHitDuration);
		}
		...
		void ShootEffect(Vector3 pos, Quaternion rot, Transform shootPoint){
			...
			pooledObject.GetPoolItem(pos, rot, shootPoint).ReturnToPool(destroyShootDuration);
		}
		...
		void FireSplitSO(Quaternion shootRot){
			...
			ShootObject soInstance = GetPoolItem<ShootObject>(thisT.position, shootRot);
